...
el("interest-submit-btn").onclick = async () => {
  ...
  const playtomicName = el("interest-playtomic-name").value.trim();
  ...
  try {
    await api("/interest", { method: "POST", body: { name, playtomicName, playtomicLevel, league: league_, joinAs } });
    el("interest-name").value = ""; el("interest-playtomic-name").value = ""; el("interest-playtomic-level").value = "";
    el("interest-league").value = ""; el("interest-join-as").value = "";
  ...
  c.innerHTML = signups.map((s) => `
    <div class="notif-row" data-id="${s.id}">
      <div>
        ...
        <div class="note">Playtomic: ${escapeHtml(s.playtomicName || "—")}${s.playtomicLevel ? " (" + escapeHtml(s.playtomicLevel) + ")" : ""}</div>
      </div>
